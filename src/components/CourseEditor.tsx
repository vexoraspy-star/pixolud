...
import { COURSE_ROUND_OPTIONS, isCoursePlayable, type CourseData } from "@/lib/course";
...
export default function CourseEditor({
  ...
  useEffect(() => {
    ...
    scheduleSave({ title, description, data: course, gradient, emoji, coverUrl, multiplayerMode });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [title, description, course, gradient, emoji, coverUrl, multiplayerMode]);
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8">
        <p className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">
          Nombre de manches (test de réflexes)
        </p>
        <div className="flex gap-2">
          {COURSE_ROUND_OPTIONS.map((r) => (
            <button
              key={r}
              type="button"
              onClick={() => setCourse({ rounds: r })}
        ...
        <p className="mt-2 text-xs text-zinc-400">
          Le joueur devra appuyer sur ESPACE le plus vite possible dès que le
          signal passe au vert, {course.rounds} fois de suite.
        </p>
      </div>
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <form action={publishGame}>
          ...
          <button
            type="submit"
            disabled={!isCoursePlayable(course)}
