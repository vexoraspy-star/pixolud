...
export default function EducationEditor({
  ...
  function addQuestion() {
    setData((prev) => ({ ...prev, questions: [...prev.questions, newEducationQuestion()] }));
  }
  ...
  function updateOption(id: string, index: number, text: string) {
    setData((prev) => ({
      ...
      questions: prev.questions.map((q) =>
        q.id === id ? { ...q, options: q.options.map((o, i) => (i === index ? text : o)) } : q,
      ),
    }));
  }
  ...
  function setCorrect(id: string, index: number) {
    setData((prev) => ({
      ...prev,
      questions: prev.questions.map((q) => (q.id === id ? { ...q, correctIndex: index } : q)),
    }));
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-8">
        <p className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">Matière</p>
      ...
      <div className="mt-6">
        <p className="mb-2 text-sm font-medium text-zinc-700 dark:text-zinc-200">
          Cours (optionnel) — affiché avant les questions
        </p>
        <textarea
          value={data.lesson}
          onChange={(e) => setData((prev) => ({ ...prev, lesson: e.target.value }))}
          placeholder="Résume ici les points importants à retenir..."
          rows={5}
      ...
      <div className="mt-8 flex flex-col gap-6">
        {data.questions.map((q, qi) => (
          <div key={q.id} className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-semibold text-zinc-400">Question {qi + 1}</span>
              <button
                type="button"
            ...
            <p className="mt-2 text-xs text-zinc-400">Coche la bonne réponse à gauche.</p>
          </div>
        ))}
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        <form action={publishGame}>
          ...
          <button
            type="submit"
            disabled={!isEducationPlayable(data)}
